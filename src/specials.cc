 ...
 * @param[in]  defscale
 **/
char * store_graph_positional_args(
    struct graph * g,
    char * quoted_cmd,
    char * argstr,
    double defscale) {
...
const char *scan_gauge(struct text_object *obj, const char *args,
  ...
  if (args != nullptr) {
    ...
    char _args[1024] = {'\0'};
    strncpy(_args, args, strlen(args));
    store_option_value<int *>(&g->height, "%d", _args, NAMED_ARG_PREFIX "height");
    store_option_value<int *>(&g->width, "%d", _args, NAMED_ARG_PREFIX "width");
  }
...
const char *scan_bar(struct text_object *obj, const char *args, double scale) {
  ...
  if (args != nullptr) {
    ...
    char _args[1024] = {'\0'};
    strncpy(_args, args, strlen(args));
    store_option_value<int *>(&b->height, "%d", _args, NAMED_ARG_PREFIX "height");
    store_option_value<int *>(&b->width, "%d", _args, NAMED_ARG_PREFIX "width");
  }
...
char *scan_graph(struct text_object *obj, const char *args, double defscale) {
  ...
  char argstr[1024] = {'\0'};     /* args minus quoted_cmd and named arguments */
  ...
  if (args != nullptr) {
    ...
    /* copy everything before the first named argument into argstr
     * This means all named arguments must be after the positional
     * arguments */
    size_t length = 1023;
    char * end = strstr(no_quote_args, NAMED_ARG_PREFIX);
    if (end != nullptr) {
    ...
    char * ptr = store_graph_positional_args(g, quoted_cmd, argstr, defscale);

    /* process the named arguments */
    /* Make sure to add a new template declaration under
     * store_option_value() if adding a call to it with a new type */
    store_option_value<int *> (&g->height, "%d", no_quote_args, NAMED_ARG_PREFIX "height");
    store_option_value<int *> (&g->width, "%d", no_quote_args, NAMED_ARG_PREFIX "width");
    store_option_value<double *> (&g->scale, "%lf", no_quote_args, NAMED_ARG_PREFIX "scale");
    store_option_value<unsigned int *> (&g->first_colour, "%x", no_quote_args, NAMED_ARG_PREFIX "first_colour");
    store_option_value<unsigned int *> (&g->last_colour, "%x", no_quote_args, NAMED_ARG_PREFIX "last_colour");
    store_option_value<unsigned int *> (&g->scale_colour, "%x", no_quote_args, NAMED_ARG_PREFIX "scale_colour");
